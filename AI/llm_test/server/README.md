...
백엔드 연동용 메인 엔드포인트. STT 텍스트를 받아 LLM으로 의미 추론 후 반환.
...
**Response** — `LLMResponse` 전체 필드 반환 (토큰 수, 비용 포함)
...
| intent | 설명 |
|--------|------|
| `navigation` | 경로 탐색 요청 (출발지·도착지 추출) |
| `phone` | 전화 요청 |
...
│   ├── base_provider.py    # 공통 인터페이스 및 LLMResponse 데이터클래스
│   ├── utils.py            # SYSTEM_PROMPT, JSON 파싱 유틸
│   ├── gemini_provider.py
│   ├── claude_provider.py
│   └── gpt_mini_provider.py
│
├── utils/                  # 공통 유틸리티
│   ├── logger.py           # 로깅 설정
...
│   ├── intent_parser.py    # 정규식 기반 intent 파싱
│   └── stt_service.py      # STT 서비스
│
└── tests/
    ├── test_batch.py       # 3개 모델 배치 테스트
    ├── results/            # API 호출 결과 JSON 저장소
...
결과 파일은 `tests/test_results/batch_test_YYYY-MM-DD_HH-MM-SS.md`에 저장된다.

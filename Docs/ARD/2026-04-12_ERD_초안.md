...
- `category`: `RESTAURANT`, `TOURIST_SPOT`, `TOILET`, `BUS_STATION`, `CHARGING_STATION`, `BARRIER_FREE_FACILITY`, `ACCOMMODATION`
...
- 장소 자체가 `CHARGING_STATION` 같은 시설인 경우 같은 feature를 중복 저장하지 않는다.
- `elevator`는 장소의 부가 접근성 속성으로 관리한다.

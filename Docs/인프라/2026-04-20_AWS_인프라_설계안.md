...
| 서버 | 역할 | 항상 실행 여부 | 비고 |
|---|---|---|---|
| `S1` | `blue(prod)`, `dev`, `Jenkins`, `Portainer`, `GraphHopper runtime(serve only)` | 항상 실행 | 현재 핵심 운영 서버. Graph import/build는 제외 |
| `S2` | `green(prod standby)`, `GraphHopper runtime(serve only)`, `PLG(Grafana 포함)` | 항상 실행 | 배포/장애 전환 후보. green 승격 시 `PLG`는 중지 가능 |
...
| Portainer | `portainer.example.com` | `ALB -> S1 Portainer target group` |
...
flowchart LR
    ...
    ALB -->|"Host=portainer.example.com + Admin IP"| PORTAINER["S1 Portainer"]
...
- `S2`가 `green standby`이면서 동시에 `PLG`를 운영하면 장애 전환 시 자원 충돌 가능성이 있다.
...
- `S1` 장애 시 `prod 주 서버 + dev + Jenkins + Portainer`가 함께 영향을 받는다.
- ALB가 살아 있더라도 `S1`에만 있는 Jenkins/Portainer는 `S1` 장애 시 사용할 수 없다.
...
- `모니터링 1차 = CloudWatch`
- `모니터링 2차 = S2의 PLG`
- `Jenkins`, `Grafana`, `Portainer`는 ALB의 **host-based routing**으로 웹 접근을 허용하되, 원 포트는 외부에 공개하지 않는다.

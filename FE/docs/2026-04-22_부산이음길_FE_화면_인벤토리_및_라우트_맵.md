...
| 구현 메모 | 제보 유형은 `계단/단차가 없다고 나와있는데 실제로 있음`, `점자블록`, `인도라고 적혀 있는데 실제로 없음`, `경사로`, `인도폭` 5개를 API enum 후보로 둔다. 사진은 imageUrl 선업로드 방식이며 실제 URL/API 상세는 추후 구현이다. 처리 상태는 노출하지 않는다. |
| 근거 수준 | `코드 기준 확인` + `관찰 사실` |

...
class MyPageAppInfoRouteTest {
    ...
    fun `privacy policy intent opens the configured notion page`() {
        val intentSpec = privacyPolicyIntentSpec()

        assertEquals(Intent.ACTION_VIEW, intentSpec.action)
        assertEquals(
            "https://www.notion.so/ryuwon-project/350a58d49be68063bbd1f633be85badb",
            intentSpec.dataString,
        )
        assertTrue(intentSpec.flags and Intent.FLAG_ACTIVITY_NEW_TASK != 0)
    }

    @Test
    fun `service terms intent opens the configured notion page`() {
        val intentSpec = serviceTermsIntentSpec()

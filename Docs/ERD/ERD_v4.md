...
| 한글명 | 영어명 | 타입 | NULL | DEFAULT |
| --- | --- | --- | --- | --- |
| 자주 가는 길 ID | fav_route_id | INT | NOT NULL |  |
...
- 카카오 장소 ID를 내부 장소와 매칭 근거로 채택한 경우 `provider=KAKAO`, `provider_place_id`에 저장할 수 있다. DB 기본키인 `place_id`는 내부 자동 증가 ID로 유지한다.
- `provider`, `provider_place_id` 조합에 유니크 제약을 둔다. 단, 카카오 검색 결과를 북마크했다는 이유만으로 `places`를 자동 생성하지 않는다.

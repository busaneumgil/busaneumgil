...
- 작성일: 2026-04-22
- 최종 개정일: 2026-04-28
- 기준 문서: 실제 `FE/app` navigation/screen 코드, `FE/mockup/`, `FE/docs/2026-04-21_S14P31E102-211_공통_UI_접근성_Figma_handoff.md`, `Docs/기획/2026-04-09_화면명세서.md`, `Docs/기획/2026-04-11_MVP_화면명세서.md`
- 보조 정합성 참고: `FE/docs/2026-04-27_로그인_필수_전환_FE_정합성_및_구현_영향.md`, `FE/docs/2026-04-27_온보딩_디자인_화면_구성_분석.md`, `FE/docs/2026-04-27_FE_Docs_문서_정합성_재검토.md`
- 대상: 부산이음길 Android FE 구현 / route 설계 / 화면 범위 고정
- 범위: 현재 워크스페이스의 `FE/app`, `FE/docs`, `FE/mockup`에서 확인 가능한 화면만 정리
- 정합성 원칙: 이 문서는 FE 화면/route의 1차 계약 문서다. 화면명, IA, 사용자 노출 용어, route 목표 구조는 이 문서와 동급 1차 FE 문서를 우선한다. 현재 구현 사실 확인은 `FE/app` 코드가 우선하며, 2차 정리 문서는 이 문서를 덮어쓰지 않는다. production 기준 정책은 `AUTH-001` 필수 로그인, `ONB-001~ONB-005` 5단계 온보딩, 서버 기준 데이터 책임을 따른다. 보행약자 공통 화면은 `홈/북마크/제보/마이페이지` 탭을 사용하고, 저시력자 전용 UI는 공통 화면의 모드 예외가 아니라 별도 `LV-*` 화면군으로 분리한다.
...
- 이 문서는 `목표 화면 계약`과 `현재 구현 사실`을 분리해 정리한다.
  ...
  - 4순위: `FE/docs/2026-04-27_로그인_필수_전환_FE_정합성_및_구현_영향.md`, `FE/docs/2026-04-27_온보딩_디자인_화면_구성_분석.md`, `FE/docs/2026-04-27_FE_Docs_문서_정합성_재검토.md` 같은 2차 정리 문서
...
- 직접 산출물: `FE/docs/2026-04-21_S14P31E102-211_공통_UI_접근성_Figma_handoff.md`, `Docs/기획/2026-04-09_화면명세서.md`, `Docs/기획/2026-04-11_MVP_화면명세서.md`
- 2차 정리 문서: `FE/docs/2026-04-27_로그인_필수_전환_FE_정합성_및_구현_영향.md`, `FE/docs/2026-04-27_온보딩_디자인_화면_구성_분석.md`, `FE/docs/2026-04-27_FE_Docs_문서_정합성_재검토.md`, `FE/docs/sprint_backlog/2026-04-21_S14P31E102-204_제보_작성_폼_state_spec.md`
...
| `ONB-002` | 저시력자 전용 후속 화면 | 온보딩 | 저시력자 분기 전용 후속 단계 제공 | `ONB-001`에서 `저시력자` 선택 | N | `다음` | `P0` | 세부 UI 추후 확정 |
| `ONB-003` | 보행약자 2차 선택 | 온보딩 | 보행약자 세부 이동 특성 선택 | `ONB-001`에서 `보행약자` 선택 | N | 선택 완료 후 다음 단계 | `P0` | `전동휠체어 사용자 / 수동휠체어 사용자 / 기타 보행약자` |
| `ONB-004` | 약관/동의 | 설정/약관 | 필수 동의 수집 후 OS 위치 권한 요청으로 연결 | `ONB-002` 또는 `ONB-003` 완료 | N | `다음` | `P0` | 약관 동의와 OS 위치 권한 요청 분리 |
| `ONB-005` | 튜토리얼 | 온보딩 | 위치 권한 응답 이후 핵심 사용 흐름 안내 후 메인 진입 | `ONB-004` 다음 + OS 위치 권한 응답 완료 | N | `다음`, `시작하기` | `P0` | 기존 서비스 소개 슬라이드 재배치 |
| `LV-ONB-01` | 저시력자 약관 동의 | 저시력자 전용 온보딩 | 1번째 순차 동의 화면 | `ONB-002` 완료 | N | 중앙 대형 버튼 더블탭 | `P0` | `약관 동의` |
...
| `LV-HOM-01` | 저시력자 홈 | 저시력자 전용 메인 | 음성 입력과 현재 위치 중심 홈 | `ONB-005` 완료 후 `저시력자` 분기 | Y | 음성 입력, 현재 위치 | `P0` | 하단 탭 `홈/북마크/카테고리/마이페이지` |
...
| `MYP-01` | 마이페이지 홈 | 마이페이지 | 프로필, 배지, 메뉴, 계정 액션 | 하단 탭 `마이페이지` | Y | 사용자 유형 변경, 로그아웃 | `P1` | 사용자 유형 변경은 `ONB-001` |
...
| `REPORT` | `ic_report_construction`, `ic_report_stairs`, `ic_report_slope`, `ic_report_ramp`, `ic_report_elevator`, `ic_report_tactile_damage`, `ic_report_guidance_block`, `ic_report_facility_damage`, `ic_report_obstacle`, `ic_report_other`, `ic_permission_camera`, `ic_status_check` | 제보 유형 8개와 리소스명은 API enum 확정 전까지 문서 기준으로 고정한다. |
...
| 이탈 경로 | 로그인 성공 + `onboardingCompleted=false` -> `ONB-001`<br/>로그인 성공 + `onboardingCompleted=true` + `selectedPrimaryUserType=LOW_VISION` -> `LV-HOM-01`<br/>로그인 성공 + `onboardingCompleted=true` + `selectedPrimaryUserType=MOBILITY_IMPAIRED` -> `MAP-01` |
...
| 필요한 데이터 | `socialProvider`, `socialAccessToken`, `authSession`, `onboardingCompleted` |
| 필요한 파라미터 | 없음 |
...
| 진입 조건 | 인증 완료 + `onboardingCompleted == false` |
| 진입 경로 | 앱 최초 실행 gate 통과 후 또는 온보딩 미완료 상태 |
| 이탈 경로 | `저시력자` 선택 -> `ONB-002`<br/>`보행약자` 선택 -> `ONB-003` |
...
| 목적 | `저시력자` 분기 사용자를 위한 전용 후속 단계를 제공한다. |
| 사용자가 이 화면에서 하는 일 | 전용 후속 안내/설정 확인 후 `다음` 선택 |
| 진입 조건 | `ONB-001`에서 `저시력자` 선택 |
...
| 구현 메모 | 별도 전용 후속 화면의 존재와 진입/이탈만 고정하고 세부 UI는 추후 확정으로 둔다. |
| 근거 수준 | `제품 기준선` |
...
| 목적 | 서비스 이용을 위한 필수 법적 동의를 수집하고 OS 위치 권한 요청으로 연결한다. |
| 사용자가 이 화면에서 하는 일 | 필수 항목 확인 및 동의 후 `다음` 선택 |
| 진입 조건 | `ONB-002` 또는 `ONB-003` 완료 |
| 진입 경로 | `ONB-002` -> `ONB-004` 또는 `ONB-003` -> `ONB-004` |
...
| 필요한 데이터 | `termsAgreed`, `locationPermissionStatus` |
| 필요한 파라미터 | 없음 |
| 주요 상태 | `default`: 미동의<br/>`completed`: 필수 약관 동의 완료 |
| 접근성/모드 차이 | 각 항목의 필수 여부와 현재 체크 상태를 항목별로 읽을 수 있어야 한다. |
| 구현 메모 | `개인위치정보 수집·이용 동의`는 법적 동의 항목이고, OS 위치 권한 요청은 `다음` 이후 시스템 팝업이다. 저시력자(시각장애인) 전용 UI에서는 `약관 동의 -> 민감정보 -> 위치정보 -> 14세 이상 -> 처리방침` 5개 화면을 한 항목씩 순차 노출하고, 중앙 대형 버튼 더블탭으로 다음 항목으로 진행하는 패턴을 허용한다. |
| 근거 수준 | `제품 기준선` |
...
| 화면 ID | `ONB-005` |
| 화면명 | 튜토리얼 |
| route / route key | `onboarding/tutorial` (`OnboardingRoute.Tutorial`) |
| 목적 | 위치 권한 응답 이후 핵심 기능을 짧게 안내하고 메인으로 진입시킨다. |
| 사용자가 이 화면에서 하는 일 | 튜토리얼 카드 확인 후 `다음` 또는 `시작하기` 선택 |
| 진입 조건 | `ONB-004` 다음 선택 후 OS 위치 권한 응답 완료 |
| 진입 경로 | `ONB-004` -> 시스템 팝업 -> `ONB-005` |
| 이탈 경로 | 마지막 페이지 `시작하기` -> `selectedPrimaryUserType`에 따라 `LV-HOM-01` 또는 `MAP-01` |
...
| 필요한 데이터 | `tutorialPage`, `onboardingCompleted` |
| 필요한 파라미터 | 없음 |
| 주요 상태 | `default`: 소개 페이지 표시<br/>`completed`: 온보딩 완료 저장 후 메인 이동 |
| 접근성/모드 차이 | 기능 카드의 아이콘 의미는 텍스트 제목과 설명으로 중복 제공한다. |
| 구현 메모 | 서비스 소개는 3페이지 이상으로 구성한다. 다만 저시력자 전용 UI는 공통 `MAP-01` 축약 흐름이 아니라 별도 `LV-*` 화면군으로 분리하고, `ONB-005` 완료 후 `LV-HOM-01`로 연결한다. |
| 근거 수준 | `신규 디자인 기준` |
...
| 구현 메모 | `MAP-01`은 보행약자 공통 홈으로 유지한다. 저시력자 전용 홈/카테고리/결과는 `LV-HOM-01`, `LV-CAT-01`, `LV-RES-01` 별도 화면군으로 분리한다. 현재 코드의 과거 탭 구조는 구현 alias로만 본다. |
...
| 사용자가 이 화면에서 하는 일 | 검색어 입력, STT 장소 검색, 최근 방문지 선택, 결과 목록 선택, 지도 바텀시트 확인. 저시력자 전용 UI에서는 2개 대형 카드 결과와 카드별 `북마크`, `길찾기`를 우선 사용한다. |
| 진입 조건 | 지도 검색 진입 클릭 |
| 진입 경로 | `MAP-01` 검색바 |
...
| 구현 메모 | `DEST-01`~`DEST-04`는 보행약자 공통 목적지 설정 화면군이다. 현재 code route `search`는 `DEST-01/02` 구현 alias다. 저시력자 전용 검색/카테고리 결과는 `LV-RES-01`을 사용하고 `길찾기` CTA는 `LV-NAV-01`로 직접 진입한다. |
| 근거 수준 | `코드 기준 확인` + `관찰 사실` |
...
| 진입 조건 | 하단 탭 `마이페이지` 선택 |
| 진입 경로 | 하단 탭 `마이페이지` |
| 이탈 경로 | 사용자 유형 변경 -> `ONB-001`<br/>제보 내역 -> `MYP-02`<br/>앱 정보 및 도움말 -> `MYP-03`<br/>로그아웃 -> `AUTH-001` |
...
| 구현 메모 | 편집 모드에서는 삭제 기능만 제공한다. 정렬, 전체 선택, 다중 선택은 제공하지 않는다. 저시력자 전용 북마크는 `BMK-01`의 모드 예외가 아니라 `LV-BMK-01` 별도 화면으로 분리하고, 카드의 빈 영역 더블탭은 `LV-BRF-01`로 연결한다. |
| 근거 수준 | `코드 기준 확인` + `관찰 사실` |

### 저시력자 관련 레거시 목업 메모

- 과거 목업의 `VIS-01`~`VIS-04`는 현재 MVP 메인 route ID가 아니다.
- 현재 기준선에서 저시력자 관련 메인 온보딩 흐름은 `ONB-001` 1차 선택의 `저시력자` 분기 후 `ONB-002 저시력자 전용 후속 화면`으로 연결된다.
...
| `ONB-001` | `저시력자` 선택 완료 | `ONB-002` | `selectedPrimaryUserType == LOW_VISION` | `selectedPrimaryUserType` | 1차 분기 저장 |
| `ONB-001` | `보행약자` 선택 완료 | `ONB-003` | `selectedPrimaryUserType == MOBILITY_IMPAIRED` | `selectedPrimaryUserType` | 1차 분기 저장 |
| `ONB-002` | `다음` | `ONB-004` | 저시력자 전용 후속 단계 확인 완료 | 없음 | 세부 UI는 추후 확정 |
| `ONB-003` | 세부 유형 선택 완료 | `ONB-004` | `selectedMobilitySubtype` 저장 | `selectedMobilitySubtype` | 2차 분기 저장 |
| `ONB-004` | `다음` | OS 위치 권한 시스템 팝업 | 필수 약관 동의 완료 | `termsAgreed` | `개인위치정보 수집·이용 동의`와 시스템 권한 요청 분리 |
| OS 위치 권한 시스템 팝업 | 허용 또는 거부 응답 | `ONB-005` | 응답 완료 | `locationPermissionStatus` | 응답 이후 튜토리얼 진입 |
| `ONB-005` | `시작하기` | `selectedPrimaryUserType`에 따라 `LV-HOM-01` 또는 `MAP-01` | 마지막 튜토리얼 페이지 완료 | 없음 | onboarding stack clear, `onboardingCompleted = true` |
...
| `MAP-01` | 하단 탭 `카테고리` | `DEST-02` 또는 카테고리 결과 subview | 저시력자 전용 UI | category id | `화장실/음식점/숙박시설/병원` 4개 카드 |
...
flowchart TD
    ...
    C -->|저시력자| D["ONB-002 저시력자 전용 후속 화면<br/>route: onboarding/low_vision_followup"]
    C -->|보행약자| E["ONB-003 보행약자 2차 선택<br/>route: onboarding/mobility_type_secondary"]
    D -->|다음| G0["ONB-004 약관/동의<br/>route: onboarding/terms"]
    E -->|선택 완료| G0
    G0 -->|다음| H0["OS 위치 권한 시스템 팝업"]
    H0 -->|응답 완료| I0["ONB-005 튜토리얼<br/>route: onboarding/tutorial"]
    I0 -->|시작하기| F
...
```mermaid
flowchart TD
    A["ONB-002 저시력자 전용 후속 화면"] --> B["LV-ONB-01 저시력자 약관 동의"]
...
- 저시력자 관련 현재 메인 온보딩 분기는 `ONB-001 -> ONB-002 -> LV-ONB-01 -> LV-ONB-02 -> LV-ONB-03 -> LV-ONB-04 -> LV-ONB-05 -> OS 위치 권한 시스템 팝업 -> ONB-005 -> LV-HOM-01`이다.
...
| `MAP-01`, `DEST-01`~`DEST-04`, `ARR-01`, `BMK-01`~`BMK-02`, `REPORT-01`~`REPORT-04`, `MYP-01`~`MYP-03` | 표시 | 보행약자 공통 화면군 |
...
| --- | --- | --- | --- | --- | --- | --- |
| `ONB-001` | `selectedPrimaryUserType` | 필수 | `Enum` (`LOW_VISION`, `MOBILITY_IMPAIRED`) | DataStore 저장 | 누락 시 `ONB-001` 유지 | `제품 기준선` + `추정` |
| `ONB-002` | 저시력자 전용 후속 단계 완료 여부 | 필수 | `Boolean` | 화면 state 또는 DataStore 후보 | 누락 시 `ONB-002` 유지 | `제품 기준선` + `추정` |
| `ONB-003` | `selectedMobilitySubtype` | 필수 | `Enum` (`POWER_WHEELCHAIR`, `MANUAL_WHEELCHAIR`, `OTHER_MOBILITY_IMPAIRED`) | DataStore 저장 | 누락 시 `ONB-003` 유지 | `제품 기준선` + `추정` |
| `ONB-004` | `termsAgreed`, `locationPermissionStatus` | 필수 | `Boolean`, permission status enum | 동의 상태 저장 + 시스템 팝업 응답 반영 | 필수 항목 미동의 시 CTA disabled | `제품 기준선` + `추정` |
| 앱 시작 분기 | `onboardingCompleted`, `selectedPrimaryUserType`, `selectedMobilitySubtype`, 필수 약관 동의 여부 | 필수 | DataStore 저장값 | 초기 설정 조회 | 미완료 시 `ONB-001` 또는 마지막 미완료 단계로 분기 | `제품 기준선` + `추정` |
...
| `onboarding/user_type_primary` | `ONB-001` | 사용자 유형 1차 선택 | 목표 기준, 구현 개정 필요 |
| `onboarding/low_vision_followup` | `ONB-002` | 저시력자 전용 후속 화면 | 목표 기준, 구현 개정 필요 |
| `onboarding/mobility_type_secondary` | `ONB-003` | 보행약자 2차 선택 | 목표 기준, 구현 개정 필요 |
| `onboarding/terms` | `ONB-004` | 약관 동의 | 목표 기준, 구현 개정 필요 |
| `onboarding/tutorial` | `ONB-005` | 튜토리얼 | 목표 기준, 구현 개정 필요 |
| `lv/onboarding/terms_service` | `LV-ONB-01` | 저시력자 약관 동의 | 목표 기준, 신규 구현 필요 |
| `lv/onboarding/terms_sensitive` | `LV-ONB-02` | 저시력자 민감정보 동의 | 목표 기준, 신규 구현 필요 |
| `lv/onboarding/terms_location` | `LV-ONB-03` | 저시력자 개인위치정보 동의 | 목표 기준, 신규 구현 필요 |
| `lv/onboarding/terms_age` | `LV-ONB-04` | 저시력자 만 14세 이상 확인 | 목표 기준, 신규 구현 필요 |
| `lv/onboarding/privacy_notice` | `LV-ONB-05` | 저시력자 개인정보 처리방침 확인 | 목표 기준, 신규 구현 필요 |
| `lv/home` | `LV-HOM-01` | 저시력자 홈 | 목표 기준, 신규 구현 필요 |
| `lv/search_voice` | `LV-SEA-01` | 저시력자 음성 입력 진행 | 목표 기준, 신규 구현 필요 |
| `lv/result_list` | `LV-RES-01` | 저시력자 장소 결과 목록 | 목표 기준, 신규 구현 필요 |
| `lv/category` | `LV-CAT-01` | 저시력자 카테고리 | 목표 기준, 신규 구현 필요 |
| `lv/bookmark` | `LV-BMK-01` | 저시력자 북마크 | 목표 기준, 신규 구현 필요 |
| `lv/route_briefing` | `LV-BRF-01` | 저시력자 경로 브리핑 | 목표 기준, 신규 구현 필요 |
| `lv/navigation` | `LV-NAV-01` | 저시력자 길안내 진행 | 목표 기준, 신규 구현 필요 |
| `lv/arrival` | `LV-ARR-01` | 저시력자 도착 완료 | 목표 기준, 신규 구현 필요 |
| `lv/my_page` | `LV-MYP-01` | 저시력자 마이페이지 | 목표 기준, 신규 구현 필요 |
...
- 앱 진입 gate는 `authSession -> onboardingCompleted -> selectedPrimaryUserType 홈 분기`로 고정하되, 시각적 첫 노출을 로그인 화면으로 둘지 온보딩 화면으로 둘지는 추가 확인 필요
- 로그인 제공자를 Kakao/Naver/Google 전체로 둘지, MVP에서는 일부만 둘지
- 온보딩과 마이페이지에서 사용할 프로필 필드 데이터 매핑: 닉네임, 사용자 이동 특성, 연락처 등 `BE 확인 필요`
...
- `ONB-002` 저시력자 전용 후속 화면의 세부 UI 항목을 어디까지 확정할지
...
1. 앱 시작 gate를 `authSession -> onboardingCompleted -> selectedPrimaryUserType 홈 분기` 순서로 고정한다.
...
11. 저시력자 분기는 `ONB-001`의 메인 분기와 `ONB-002` 후속 화면 뒤 `LV-ONB-01`~`LV-MYP-01` 별도 화면군으로 고정하고, 별도 `VIS-*` route는 현재 기준선에서 사용하지 않는다.
12. 위치 선택기와 사진 선택기를 system flow로 볼지, 앱 내부 화면으로 확장할지 결정해야 한다.

## 정합성 확인 필요 항목

1. **Bottom Navigation 구조**
   - 보행약자 공통: `홈 / 북마크 / 제보 / 마이페이지` 4탭
   - 저시력자 전용: `LV-*` 별도 화면군의 `홈 / 북마크 / 카테고리 / 마이페이지`
   - 현재 구현은 공통 탭 예외가 아니라 별도 화면군 탭 집합까지 반영하는 방향으로 개정 필요
...
7. **온보딩 route 구조 충돌**
   ...
   - 신규 디자인: `ONB-001`~`ONB-005`, `selectedPrimaryUserType`, `selectedMobilitySubtype`, 약관/동의, 튜토리얼
   - 결정: 신규 디자인 기준으로 문서를 정렬하고, 구현 route는 개정 필요 항목으로 둔다.

8. **저시력자 분기 후속 화면 세부 UI 미확정**
   - 현재 기준선: `ONB-001`에서 `저시력자` 선택 시 `ONB-002`로 이동
   - 세부 UI: 별도 전용 후속 화면의 내부 구성은 추후 확정 필요
...
10. **로그인 필수 전환에 따른 route 부재**
    ...
    - 결정: `AUTH-001`을 P0 신규 화면으로 추가하고, 로그인 이후 사용자 정보 수집은 `ONB-001`~`ONB-005` 안에서 정렬한다.

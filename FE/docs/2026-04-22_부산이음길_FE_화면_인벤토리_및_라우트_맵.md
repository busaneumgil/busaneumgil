...
- 정합성 원칙: 이 문서는 최우선 기준 문서를 보조하는 FE 구현 참고 문서이며, production 기준 정책은 `AUTH-001` 필수 로그인, `ONB-001~ONB-005` 5단계 온보딩, `홈/북마크/제보/마이페이지` 4탭, 서버 기준 데이터 책임을 따른다.
...
| `VIS-01` | 시각 지원 확장 대화형 목적지 검색 | 검색/목적지 설정 | 시각 지원 확장 흐름의 음성 목적지 입력 | 진입점 미정 | N | `목적지 말하기` | `P2` | mockup-only |
...
| `VIS-04` | 시각 지원 확장 안내 진행 | 길 안내 | 시각 지원 확장 흐름의 안내 진행 | `VIS-03` | N | `안내 종료` | `P2` | mockup-only |
...
| 이탈 경로 | 로그인 성공 + `onboardingCompleted=false` -> `ONB-001`<br/>로그인 성공 + `onboardingCompleted=true` -> `MAP-01` |
...
| 접근성/모드 차이 | 선택 카드는 선택 여부를 `stateDescription`으로 전달한다. 카드 라벨은 `보행약자` 상위 용어와 `휠체어 사용자 (전동)`, `휠체어 사용자 (수동)`, `기타 보행약자` 세부 이동 특성을 따른다. |
| 구현 메모 | 기존 2분류 사용자 유형과 `장애 등급` 기반 라우트는 최신 디자인과 불일치한다. |
| 근거 수준 | `신규 디자인 기준` |
...
| --- | --- |
| 화면 ID | `VIS-01` |
| 화면명 | 시각 지원 확장 대화형 목적지 검색 |
| route / route key | 코드 route 없음. `추정 route key: 추가 결정 필요` |
| 목적 | 시각 지원 확장 흐름에서 voice-first로 목적지를 입력한다. |
| 사용자가 이 화면에서 하는 일 | 음성 버튼 탭 후 목적지 말하기 |
| 진입 조건 | 시각 지원 확장 흐름 전용 진입점 필요 |
...
| 화면명 | 시각 지원 확장 안내 진행 |
| route / route key | 코드 route 없음. `추정 route key: 추가 결정 필요` |
| 목적 | 시각 지원 확장 흐름의 간결한 실시간 안내 진행 화면을 제공한다. |
...
```mermaid
flowchart TD
    A["VIS-01 시각 지원 확장 대화형 목적지 검색"] -->|음성 입력 완료| B["VIS-02 목적지 확인"]
    B -->|네| C["VIS-03 경로 브리핑"]
    B -->|아니요, 다시 말하기| A
    C -->|안내 시작| D["VIS-04 시각 지원 확장 안내 진행"]
    D -->|안내 종료| A
```
...
- 앱 진입 gate는 `authSession -> onboardingCompleted -> MAP-01`로 고정하되, 시각적 첫 노출을 로그인 화면으로 둘지 온보딩 화면으로 둘지는 추가 확인 필요
...
- 별도 `설정 상세 트리` 화면군
- 별도 `시각 지원 확장 상세 경로 목록` route

### 9.5 FE가 먼저 고정해야 하는 네비게이션 결정 5~10개

1. 앱 시작 gate를 `authSession -> onboardingCompleted -> MAP-01` 순서로 고정한다.

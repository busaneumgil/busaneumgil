...
- `S2`에는 운영 WAS와 AI Flask intent server를 우선 배치하고, GraphHopper runtime은 graph-cache 아티팩트 흐름이 준비된 뒤 추가한다.
...
| `S2` | `primary prod`, `GraphHopper runtime(serve only)`, `AI Flask intent server` | 항상 실행 | AWS VPC 안의 운영 서버. RDS/Redis private 접근 기준 |
...
- AI Flask server와 backend prod 배포 경로 구성 완료
- prod GraphHopper runtime은 PostgreSQL LineString -> graph-cache build/deploy 흐름이 완료될 때까지 기본 compose 실행에서 제외
...
- `GraphHopper runtime(serve only)`은 graph-cache 아티팩트 준비 후 실행
- `AI Flask intent server` 실행
- `GraphHopper import/build` 작업은 포함하지 않음
- prod `docker-compose.prod.yml`의 GraphHopper service는 `graphhopper` profile로 분리한다.
- graph-cache build/deploy가 준비되기 전에는 기본 `docker compose up -d`에 포함하지 않는다.
- 운영도구는 S2에 배치하지 않음
...
`S1`은 SSAFY 제공 서버로 현재 AWS VPC에 속하지 않는다. Terraform은 `S1` 자체를 생성/관리하지 않고, `S1`은 Jenkins/dev/build/운영도구 서버와 S1 도메인 대상 외부 서버로만 취급한다. S1 고정 IP가 확인되면 A 레코드를 만들고, 고정 IP가 없으면 `k14e102.p.ssafy.io` 기준 CNAME 레코드를 만든다.
...
- `S2 prod metric`은 현재 `CloudWatch`를 1차 기준으로 유지하고, private scrape 또는 agent forwarding 설계가 준비되면 Grafana 2차 조회 범위를 넓힌다.
...
- `S2 = primary prod + AI Flask intent server`, GraphHopper runtime은 graph-cache 아티팩트 준비 후 profile로 추가

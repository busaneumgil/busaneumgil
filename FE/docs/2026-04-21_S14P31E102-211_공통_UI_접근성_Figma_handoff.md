...
- 운영 방식: 이 문서는 `204~210` 구현 흐름과 Figma handoff 사이의 기준 문서로 관리한다.
...
2026-04-27 신규 온보딩 디자인이 추가되면서, 온보딩 5단계(`ONB-001`~`ONB-005`)는 본 문서의 공통 접근성 문안 원칙을 적용하는 별도 handoff 대상에 포함한다.
...
| 시설 상세 / 검색 결과 | 접근성 라벨 | `출입 가능`, `단차 없음`, `경사로 있음`, `엘리베이터 있음`, `장애인 주차 가능`, `장애인 화장실 있음`, `안내시설 있음`, `객실 이용 가능`의 고정 문구를 사용한다. | 접근성 정보 칩/태그 공통 규칙 |
...
| 온보딩 | 사용자 유형 카드 | `지체장애자`, `보행약자` 문구와 선택 여부를 함께 전달한다. | `OnboardingUserTypeCard` |
| 온보딩 | 권한 카드 | 권한명, 필수/선택 여부, 제한되는 기능을 한 카드 안에서 읽을 수 있게 한다. | `OnboardingPermissionCard` |
...
| 로그인 | 기본, 소셜 로그인 진행 중, 제공자 오류, 서버 오류, 네트워크 오류 | 서비스 브랜드, 소셜 로그인 버튼, 오류 메시지, 재시도 액션 | `비로그인으로 계속` CTA는 두지 않고, 제공자명과 로그인 동작이 버튼 라벨로 읽히게 한다. |
| 프로필 설정 / 회원가입 완료 | 기본, 입력 중, 필수값 누락, 제출 중, 완료, 실패 | 닉네임 입력, 사용자 유형 선택, 선택 입력, 하단 완료 CTA | 필수 입력 여부, 오류 사유, 선택 상태를 TalkBack으로 전달한다. |
| 온보딩 | 스플래시, 사용자 유형 선택 전/후, 권한 안내, 권한 거부, 약관 미동의/동의, 서비스 소개 마지막 페이지 | 브랜드 영역, 사용자 유형 카드, 권한 카드, 약관 카드, 체크박스, 기능 소개 카드, 하단 CTA, page indicator | `ONB-001`~`ONB-005` 흐름과 CTA 문구가 최신 디자인 기준과 일치하는지 확인한다. |
...
- Figma와 FE fixture 데이터 모두 아래 문구를 그대로 사용한다.
  ...
  - `안내시설 있음`
  - `객실 이용 가능`
- 검색 결과 카드와 시설 상세 바텀시트에서는 `최대 3개`까지만 노출하고, 초과분은 `+n` 칩으로 처리한다.

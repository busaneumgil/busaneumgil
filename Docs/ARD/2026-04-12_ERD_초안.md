...
| 장애 유형 | disability_type | ENUM | NOT NULL | UNSET |
...
- `disability_type`: `VISUAL`, `MOBILITY`, `UNSET`
...
- 최초 소셜 로그인 직후에는 프로필 입력이 완료되지 않았을 수 있으므로 `nickname`은 `NULL`을 허용하고, `disability_type`은 `UNSET`을 기본값으로 둔다.
- 회원가입 완료 상태는 `nickname IS NOT NULL`이고 `disability_type != UNSET`인 경우로 판단한다.
...
| 사용자 ID | userId | UUID | NULL |  |
| 익명 클라이언트 ID | anonymousClientId | UUID | NULL |  |
| 장애 유형 | disability_type | ENUM | NOT NULL | UNSET |
| 경로 종류 | route_option | ENUM | NOT NULL | SAFE |
...
- 로그인 사용자는 토큰의 `userId`를 저장하고, 비로그인 사용자는 `anonymousClientId`를 저장한다.
- `userId`와 `anonymousClientId` 중 하나 이상은 있어야 한다.
...
- 한 사용자는 여러 제보를 등록할 수 있다.

### users - route_logs

- `users 1 : N route_logs`
- 로그인 사용자는 여러 실제 이동 경로 로그를 가질 수 있다.
- 비로그인 사용자의 로그는 `anonymousClientId` 기준으로 저장하므로 `users`와 연결되지 않을 수 있다.

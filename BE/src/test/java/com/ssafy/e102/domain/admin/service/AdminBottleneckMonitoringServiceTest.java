...
class AdminBottleneckMonitoringServiceTest {
	...
	void getBottleneckMonitoring() {
		...
		AdminBottleneckMonitoringService service = new AdminBottleneckMonitoringService(queryRepository);
		...
		assertThat(response.table().rows().get(0).location()).isEqualTo("신호동-명지1동 이동축");

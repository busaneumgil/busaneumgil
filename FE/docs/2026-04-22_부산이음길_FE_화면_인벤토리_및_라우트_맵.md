...
| 앱 시작 분기 | `onboardingCompleted`, `selectedPrimaryUserType`, 조건부 `selectedMobilitySubtype`, 필수 약관 동의 여부 | 필수 | 서버 사용자 설정 조회 후 FE 상태로 매핑 | 초기 설정 조회 | 미완료 시 `ONB-001` 또는 마지막 미완료 단계로 분기 | `제품 기준선` |
...
7. **온보딩 route 구조 충돌**
   ...
   - 신규 디자인: `ONB-001`~`ONB-005`, `selectedPrimaryUserType`, 조건부 `selectedMobilitySubtype`, 약관/동의, 튜토리얼

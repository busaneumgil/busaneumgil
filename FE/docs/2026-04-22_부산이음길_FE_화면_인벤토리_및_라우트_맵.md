...
| `REPORT-01` | 장애물 유형 선택 | 제보 | 5개 유형 중 선택 | 하단 탭 `제보` | Y | 유형 선택 | `P1` | API enum 후보 5개 |
...
| `REPORT` | `ic_report_stairs`, `ic_report_tactile_damage`, `ic_route_sidewalk`, `ic_report_ramp`, `ic_route_roadway`, `ic_permission_camera`, `ic_status_check` | 제보 유형 5개와 리소스명은 API enum과 1:1로 유지한다. |
...
| 이탈 경로 | 기존 회원 로그인 성공 + `selectedPrimaryUserType=LOW_VISION` -> `LV-HOM-01`<br/>기존 회원 로그인 성공 + `selectedPrimaryUserType=MOBILITY_IMPAIRED` -> `MAP-01`<br/>신규 사용자 `signupToken` 발급 -> `ONB-001` |
...
| 사용자가 이 화면에서 하는 일 | 5개 유형 중 선택, pin drag로 위치 확정, 갤러리/카메라 사진 첨부, 설명 입력, 제출, 제보 내역 확인 |
| 진입 조건 | 하단 탭 `제보` 선택 |
...
| 구현 메모 | 제보 유형은 `계단·단차 있음`, `점자블록 문제`, `인도 없음`, `경사로 문제`, `인도폭 문제` 5개를 API enum 후보로 둔다. 각 카드에는 제목과 보조 설명을 함께 표시한다. 사진은 imageUrl 선업로드 방식이며 실제 URL/API 상세는 추후 구현이다. 처리 상태는 노출하지 않는다. |
| 근거 수준 | `코드 기준 확인` + `관찰 사실` |
...
| 앱 시작 | 초기 설정 검사 | `LV-HOM-01` | `authSession` 있음 + `selectedPrimaryUserType == LOW_VISION` | 없음 | 저시력자 start destination |
| 앱 시작 | 초기 설정 검사 | `MAP-01` | `authSession` 있음 + `selectedPrimaryUserType == MOBILITY_IMPAIRED` | 없음 | 보행약자 start destination |
...
| `ONB-005` | `시작하기` | `selectedPrimaryUserType`에 따라 `LV-HOM-01` 또는 `MAP-01` | 마지막 튜토리얼 페이지 완료 + `POST /auth/signup` 성공 | 없음 | signup stack clear, authSession 저장 |
...
| `REPORT-01` | 장애물 유형 선택 | `REPORT-02` | 유형 선택 | `reportType` | 5개 enum 후보 |
...
- 앱 진입 gate는 기존 회원 기준 `authSession -> selectedPrimaryUserType 홈 분기`로 고정하되, 신규 사용자는 `signupToken -> 온보딩 -> POST /auth/signup` 흐름으로 분리한다.

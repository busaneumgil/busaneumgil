...
class VoiceAssistantOverlayTest {
    ...
    @Test
    fun `search action copy keeps assistant framing and query as supporting text`() {
        assertEquals(
            ...
            resolveVoiceAssistantOverlayActionCopy(
                VoiceAssistantAction.SearchPlace(query = "부산역"),
            ),
        )
    }

    @Test
    fun `navigation stop action copy is confirmation focused`() {
        assertEquals(
            VoiceAssistantOverlayActionCopy(
            ...
            resolveVoiceAssistantOverlayActionCopy(VoiceAssistantAction.StopNavigation()),
        )
    }
}
